...
type props = {
  ...
  changeMat: React.Dispatch<React.SetStateAction<any>>,
  ...
  mat: any,
  onMouseMove: (p: any) => null,
  onMouseUp: (p: any) => null,
  onMouseDown: (p: any) => null,
  dragging: boolean,
}
...
const useMouse = ({
  ...
  const zoomIn = (direction: number | { to: number }, point: { x: number; y: number; }) => {
    ...
    let scale = typeof direction === "object" ? direction.to / mat.a : 1 + 0.2 * direction
  ...
  const mouseEvents = {
    ...
    onMouseDown: (e: MouseEvent, specialEvent: any = {}) => {
      e.preventDefault()
